...
#---------Trusted Device--------
TrustedDevice.create=INSERT INTO public.trusted_device(login_user_id, device_cd, access_type_cd, device_description, device_identifier, platform_name, platform_version, client_name, client_version, trusted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
TrustedDevice.get=SELECT t.*, dl.description AS device_locale, al.description AS access_type_locale FROM public.trusted_device AS t LEFT JOIN lookup.device_locale AS dl ON dl.device_cd = t.device_cd AND dl.locale_cd = 1 AND dl.end_time = 'infinity' LEFT JOIN lookup.access_type_locale AS al ON al.access_type_cd = t.access_type_cd AND al.locale_cd = 1 AND al.end_time = 'infinity' WHERE t.trusted_device_id = ?
TrustedDevice.getByDeviceIdentifierAndLoginUserId=SELECT t.*, dl.description AS device_locale, al.description AS access_type_locale FROM public.trusted_device AS t LEFT JOIN lookup.device_locale AS dl ON dl.device_cd = t.device_cd AND dl.locale_cd = 1 AND dl.end_time = 'infinity' LEFT JOIN lookup.access_type_locale AS al ON al.access_type_cd = t.access_type_cd AND al.locale_cd = 1 AND al.end_time = 'infinity' WHERE t.device_identifier = ? AND t.login_user_id = ? AND t.end_time = 'infinity'::timestamp ORDER BY t.trusted_device_id DESC LIMIT 1
TrustedDevice.listByLoginUserId=SELECT t.*, dl.description AS device_locale, al.description AS access_type_locale FROM public.trusted_device AS t LEFT JOIN lookup.device_locale AS dl ON dl.device_cd = t.device_cd AND dl.locale_cd = 1 AND dl.end_time = 'infinity' LEFT JOIN lookup.access_type_locale AS al ON al.access_type_cd = t.access_type_cd AND al.locale_cd = 1 AND al.end_time = 'infinity' WHERE t.login_user_id = ? AND t.end_time = 'infinity'::timestamp
TrustedDevice.updateLastSeen=UPDATE public.trusted_device SET last_seen_at = CURRENT_TIMESTAMP WHERE trusted_device_id = ?
