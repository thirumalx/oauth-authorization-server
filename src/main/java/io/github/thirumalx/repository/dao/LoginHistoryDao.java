...
public class LoginHistoryDao extends GenericDao implements LoginHistoryRepository {
	...
	private PreparedStatement setPreparedStatement(LoginHistory loginHistory, PreparedStatement ps)
			throws SQLException {
	...
	public long count(Long loginUserId) {
		Long count = jdbcTemplate.queryForObject(getSql("LoginHistory.count"),
				(ResultSet rs, int rowNum) -> rs.getLong("count"), loginUserId);
	...
	RowMapper<LoginHistory> loginHistoryRowMapper = (rs, rowNum) -> {
		...
		loginHistory.setIpAddress(rs.getObject("ip_address") != null ? rs.getString("ip_address") : null);

		loginHistory.setRowCreatedOn(
